    ...
    public partial class ExportStudentV2 : FISCA.Presentation.Controls.BaseForm, SmartSchool.API.PlugIn.Export.ExportWizard
    {
        ...
        public ExportStudentV2(string title, Image img)
        {
            ...
            _ExportableFields.ItemsChanged += delegate
            {
                ...
                List<string> newFields = new List<string>(new string[] { "學生系統編號", "學號", "班級", "姓名", "學生狀態" });
        ...
        private void wizard1_FinishButtonClick(object sender, CancelEventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                #region 取得選取學生編號
                List<JHStudentRecord> selectedStudents = JHStudent.SelectByIDs(K12.Presentation.NLDPanels.Student.SelectedSource);
                foreach (JHStudentRecord stu in selectedStudents)
                {
        ...
        void bkwNotPassComputer_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            int RowIndex = 0;
            int i = 0;
            if (studentFieldList.Contains("欄位名稱"))
                studentFieldList.Remove("欄位名稱");
            if (studentFieldList.Contains("值"))
                studentFieldList.Remove("值");
            //填表頭
                            ...
                            if (RowIndex <= 65535)
                            {
                                ...
                                for (; i < studentFieldList.Count; i++)
                                {
                                    switch (studentFieldList[i])
                                    {
                                        ...
                                        case "學生狀態": report.Worksheets[0].Cells[RowIndex, i].PutValue(x_Status(student.StatusStr)); break;
                                        default:
                                            break;
                                    }
                                }
